...
#define CUDASSERT(code)                                                        \
  {                                                                            \
    __cudassert((code), __FILE__, __LINE__);                                   \
  }
#define CUDACHECK(code)                                                        \
  {                                                                            \
    __cudassert((code), __FILE__, __LINE__, false);                            \
  }

void __cudassert(cudaError_t code, const char *file, int line,
...
uint32_t nextMul32(uint32_t x) { return ((x + 31) / 32) * 32; }
...
uint32_t getNumBlocks(const uint32_t N, const uint32_t B,
  ...
  *num_chunks = max(1, (N + min_elms_all_thds - 1) / min_elms_all_thds);
  ...
  if (num_blocks <= MAX_BLOCK) {
    ...
    // printf("Warning: reduce/scan configuration does not allow the maximal
    // concurrency supported by hardware.\n");
...
void sgmScanInc(
...

  {
    ...
    sgmScan1Block<OP>
        <<<1, block_size, shmem_size>>>(d_tmp_vals, d_tmp_flags, num_blocks);
  }
