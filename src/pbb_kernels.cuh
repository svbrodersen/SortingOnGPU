...
scanIncBlock(volatile typename OP::RedElTp *ptr, const uint32_t idx) {
  ...
  //
  // ERROR is when 1024, as warpid for thread 1023 is 31, which it writes to and
  // then for thread 31, it is attempting to read from it aswell. This could
  // lead to a read after write.
  typename OP::RedElTp sum;
  if (lane == (WARP - 1)) {
    sum = OP::remVolatile(ptr[idx]);
  }
  __syncthreads();
