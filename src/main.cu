...
template <typename T> void randomInitNat(T *data, const uint32_t size) {
...
void runBenchmarkForSize(uint32_t N, const char *typeName) {
  ...
  double elapsed = 0.0;
  {
    // Initializing with d_tmp_vals tells the sorter not to free the
    // DeviceBuffer
    ...
    for (int i = 0; i < GPU_RUNS; i++) {
      ...
      if (i == GPU_RUNS - 1) {
        if constexpr (std::is_unsigned_v<T>) {
          cudaMemcpy(sorted_array, sorter.d_out_vals_.get(), mem_size,
                     cudaMemcpyDeviceToHost);
        } else {
          cudaMemcpy(sorted_array, d_tmp_vals, mem_size,
                     cudaMemcpyDeviceToHost);
        }
      }
    }
  ...
  // This is not so fast
  if (N < 500000) {
    valid &= checkElementPreservation(inp_vals, sorted_array, N);
  }
